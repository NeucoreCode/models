    ...
    )
{
    ...
    vnn_ReleaseInceptionv2( graph, TRUE );
    if (use_img_process)
    {
        ReleaseBufferImg();
    }
...
{
    return vnn_PostProcessInceptionv2( graph );
}
    ...
    )
{
    ...
    if (use_img_process)
    {
        return vnn_PreProcessInceptionv2_ImageProcess( graph, image_name );
    }
    return vnn_PreProcessInceptionv2( graph, image_name );
}
    ...
    )
{
    ...
    tmsStart = vnn_GetPerfCount();
    graph = vnn_CreateInceptionv2(data_file_name, NULL);
