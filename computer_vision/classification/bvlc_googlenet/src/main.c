    ...
    )
{
    ...
    vnn_ReleaseBvlcGooglenetCaffe( graph, TRUE );
    if (use_img_process)
    {
        ReleaseBufferImg();
    }
...
{
    return vnn_PostProcessBvlcGooglenetCaffe( graph );
}
    ...
    )
{
    ...
    if (use_img_process)
    {
        return vnn_PreProcessBvlcGooglenetCaffe_ImageProcess( graph, image_name );
    }
    return vnn_PreProcessBvlcGooglenetCaffe( graph, image_name );
}
    ...
    )
{
    ...
    tmsStart = vnn_GetPerfCount();
    graph = vnn_CreateBvlcGooglenetCaffe(data_file_name, NULL);
