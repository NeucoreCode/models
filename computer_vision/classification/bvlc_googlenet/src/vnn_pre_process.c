...
{
    uint8_t   *bmpData;
    uint32_t   sz,file_w,file_h,file_c;
    FILE *bmpFile;
    ...
    file_c = vnn_ConvertJpegToBmpData( bmpFile, bmpData, &file_w, &file_h);
    if(width != file_w || height != file_h || channels != file_c)
    {
        printf("Wrong input jpg file\n");
        if(bmpData)
        {
            free(bmpData);
            bmpData = NULL;
...
vsi_status vnn_PreProcessBvlcGooglenetCaffe
    (
    ...
    )
{
    ...
    float mean_value[4] = {128.0,128.0,128.0,1.0};
...
*/
vsi_status vnn_PreProcessBvlcGooglenetCaffe_ImageProcess
    (
    vsi_nn_graph_t *graph,
    const char *image_name
    )
{
    ...
    if(fileType == NN_FILE_JPG)
    {
        ...
        int32_t crop_length[] = {tensor->attr.size[0], tensor->attr.size[1], 3, 1};
