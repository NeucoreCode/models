    ...
    )
{
    ...
    vnn_ReleaseInceptionv4( graph, TRUE );
    if (use_img_process)
    {
        ReleaseBufferImg();
    }
...
{
    return vnn_PostProcessInceptionv4( graph );
}
    ...
    )
{
    ...
    if (use_img_process)
    {
        return vnn_PreProcessInceptionv4_ImageProcess( graph, image_name );
    }
    return vnn_PreProcessInceptionv4( graph, image_name );
}
    ...
    )
{
    ...
    tmsStart = vnn_GetPerfCount();
    graph = vnn_CreateInceptionv4(data_file_name, NULL);
