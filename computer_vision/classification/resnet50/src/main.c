    ...
    )
{
    ...
    vnn_ReleaseResnet50Caffe( graph, TRUE );
    if (use_img_process)
    {
        ReleaseBufferImg();
    }
...
{
    return vnn_PostProcessResnet50Caffe( graph );
}
    ...
    )
{
    ...
    if (use_img_process)
    {
        return vnn_PreProcessResnet50Caffe_ImageProcess( graph, image_name );
    }
    return vnn_PreProcessResnet50Caffe( graph, image_name );
}
    ...
    )
{
    ...
    tmsStart = vnn_GetPerfCount();
    graph = vnn_CreateResnet50Caffe(data_file_name, NULL);
