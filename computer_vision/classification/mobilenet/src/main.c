    ...
    )
{
    ...
    vnn_ReleaseMobilenet( graph, TRUE );
    if (use_img_process)
    {
        ReleaseBufferImg();
    }
...
{
    return vnn_PostProcessMobilenet( graph );
}
    ...
    )
{
    ...
    if (use_img_process)
    {
        return vnn_PreProcessMobilenet_ImageProcess( graph, image_name );
    }
    return vnn_PreProcessMobilenet( graph, image_name );
}
    ...
    )
{
    ...
    tmsStart = vnn_GetPerfCount();
    graph = vnn_CreateMobilenet(data_file_name, NULL);
