    ...
    )
{
    ...
    vnn_ReleaseVgg16Caffe( graph, TRUE );
    if (use_img_process)
    {
        ReleaseBufferImg();
    }
...
{
    return vnn_PostProcessVgg16Caffe( graph );
}
    ...
    )
{
    ...
    if (use_img_process)
    {
        return vnn_PreProcessVgg16Caffe_ImageProcess( graph, image_name );
    }
    return vnn_PreProcessVgg16Caffe( graph, image_name );
}
    ...
    )
{
    ...
    tmsStart = vnn_GetPerfCount();
    graph = vnn_CreateVgg16Caffe(data_file_name, NULL);
